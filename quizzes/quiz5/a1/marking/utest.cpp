...
TEST(LaserProcessing,DetectPositon){
    ...
    bag.open(file);  // BagMode is Read by default
    sensor_msgs::LaserScan::ConstPtr laserScan = nullptr;
    nav_msgs::Odometry::ConstPtr odo = nullptr;
    ...
    for(rosbag::MessageInstance const m: rosbag::View(bag))
    {
      ...
      if( odo == nullptr){
        odo = m.instantiate<nav_msgs::Odometry>();
      }

      if ((laserScan != nullptr) && (odo != nullptr)){
        //! Now we have a laserScan and odometry so we can proceed
        //! We could also check here if we have High Intensity readings before abandoning the loop
        break;
      }
    ...
    ASSERT_NE(laserScan, nullptr);//Check that we have a laser scan from the bag
    ASSERT_NE(odo, nullptr);//Check that we have a laser scan from the bag

    ////////////////////////////////////////////
    // We insert test code below
    ...
    EXPECT_NEAR(point.x,4.3632,0.1);
    EXPECT_NEAR(point.y,-3.02028,0.1);
