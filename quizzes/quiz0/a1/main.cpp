...
int main () {
    ...
    std::cout<<"\nThe mean is: "<<stats.mean<<std::endl;
    std::cout<<"The standard deviation is: "<<stats.std_dev<<std::endl;


    //Uncomment below section to remove numbers larger than ( TASK 3 )
    removeNumbersLargerThan(myVec,stats.mean+(2*stats.std_dev));
    std::cout << "\nvector with elements over (" << stats.mean  + 2 * stats.std_dev << ") removed" << std::endl;
    printVec(myVec);
