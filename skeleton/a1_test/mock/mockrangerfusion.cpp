...
static const double MAP_RES             = 0.05;
static const double AIRSPACE_SIZE           = 30.0; //Will produce a square map of size (metres)
// The 'centre' reference for sensors is offset from zero angle (positive x axis)
static const double ANGULAR_OFFSET = 90.0;
...
void MockRangerFusion::grabAndFuseData() {
    ...
    pixel_map_size_ = static_cast<unsigned int>(AIRSPACE_SIZE / MAP_RES);
    //Step 1: Draw the airspace
    airspace_ = cv::Mat(static_cast<int>(pixel_map_size_), static_cast<int>(pixel_map_size_), CV_8UC3, CLR_MAP);

    cv::namedWindow("airspace", cv::WINDOW_NORMAL);
    cv::resizeWindow("airspace",800,800);
    ...
    for(auto cell : cells_) {
        ...
        for(;it_r < rangers_.end() && it_d < data_.end(); ++it_r, ++it_d) {
            ...
            if((*it_r)->getSensingMethod() == ranger::CONE) {
                ...
                double theta = ANGULAR_OFFSET + (sensorPose.theta*180/M_PI);
...
//                std::cout << "SONAR [r,sAngle,eAngle]" << range << "," << startAngle << "," << endAngle << std::endl;

                cv::ellipse(airspace_,convertToPoint(sensorPose),cv::Size(axis,axis),0,-startAngle,-endAngle,CLR_SONAR);

                startAngle*= M_PI/180;
                endAngle*= M_PI/180;

                //cv::circle(airspace_,convertToPoint(sensorPose),axis, CLR_SONAR);
                ranger::SensorPose sonarPoint;
                sonarPoint.x = sensorPose.x+ range*cos(startAngle);
                sonarPoint.y = sensorPose.y+ range*sin(startAngle);
                cv::line(airspace_,convertToPoint(sensorPose),convertToPoint(sonarPoint),CLR_SONAR);
            ...
            // If the sensor is a laser
                ...
                for(; it < it_d->end(); ++it, ++j) {
                    ...
                    double theta = ANGULAR_OFFSET + (sensorPose.theta*180/M_PI)  - ((*it_r)->getFieldOfView() * 0.5)
                            + (int((*it_r)->getAngularResolution()) * j);

//                    std::cout << "ANGULAR_OFFSET " << ANGULAR_OFFSET << std::endl;
//                    std::cout << "sensorPose.theta " << sensorPose.theta * 180/M_PI << std::endl;
//                    std::cout << "((*it_r)->getFieldOfView() * 0.5) " << ((*it_r)->getFieldOfView() * 0.5) << std::endl;
//                    std::cout << "(int((*it_r)->getAngularResolution()) * j) " << (int((*it_r)->getAngularResolution()) * j) << std::endl;

//                    std::cout << "elem:" << j << "angle :" << theta << std::endl;
                ...
                cv::putText(airspace_, info, {2,15}, cv::FONT_HERSHEY_SIMPLEX, 0.5, CLR_SENSOR);
            }
...
//    std::cout << "Cell cent [x,y,w]" << x << "," << y << "," << w <<
//                 " [x,y]" << tl.x << "," << tl.y <<
//                 " [x,y]" << br.x << "," << br.y << std::endl;

    cv::rectangle(airspace_,tl,br,colour);
    cv::imshow("airspace", airspace_);
//    cv::waitKey(0);
}
